...
export async function updateSettingsAction(data: unknown): Promise<ActionResult> {
  ...
  const {
    ...
    mixUrls,
    accentColor, accentColor2, heroTitle, heroTitleEn, heroTitleSize, heroContentAlign, heroOverlay, heroLayout, scrollMode, showStats,
  } = parsed.data
  ...
  await db.$transaction([
    ...
    db.djSettings.upsert({
      ...
      update: {
        ...
        accentColor, accentColor2, heroTitle: toNull(heroTitle), heroTitleEn: toNull(heroTitleEn), heroTitleSize, heroContentAlign, heroOverlay, heroLayout, scrollMode, showStats,
      },
      create: {
