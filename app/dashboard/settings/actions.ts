...
export async function updateSettingsAction(data: unknown): Promise<ActionResult> {
  ...
  const {
    ...
    accentColor, accentColor2, heroTitle, heroTitleEn, heroOverlay, heroLayout, scrollMode, showStats, galleryMode, showsMode,
  } = parsed.data
  ...
  await db.$transaction([
    ...
    db.djSettings.upsert({
      ...
      update: {
        ...
        accentColor, accentColor2, heroTitle: toNull(heroTitle), heroTitleEn: toNull(heroTitleEn), heroOverlay, heroLayout, scrollMode, showStats, galleryMode, showsMode,
      },
      create: {
