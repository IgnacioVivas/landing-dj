...
export function dbToDjPageData(dj: DjWithData): DjPageData {
  return {
    ...
    mix:      {
      urls: dj.settings?.mixUrls ?? [],
      pinnedUrl:   dj.settings?.pinnedTrackUrl   ?? null,
      pinnedTitle: dj.settings?.pinnedTrackTitle ?? null,
    },
    pressKit: {
    ...
    theme: {
      ...
      pageBackgroundOverlayOpacity: dj.settings?.pageBackgroundOverlayOpacity ?? 75,
      pageBackgroundBlur:           dj.settings?.pageBackgroundBlur           ?? 0,
