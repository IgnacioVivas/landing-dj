...
// Heights tuned for the compact player pinned to the top nav — each
// provider's own "mini" widget size, not a custom-built compact UI.
export function compactEmbedHeight(platform: EmbeddableMusicPlatform): number {
  if (platform === 'soundcloud') return 20
  if (platform === 'spotify')    return 80
  return 60
}
